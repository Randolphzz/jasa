...
from models.attention_gate import AttentionGate, AttentionGateConfig, IdentitySkip, ImprovedAttentionGate
...
class DenGCANConfig:
    ...
    skip_mode: str = "ag"  # choices: ag, iag, pwconv, cat
...
class DenGCAN(nn.Module):
    ...
    paper-specified:
        ...
        engineering assumption:
                - skip pathway supports `ag`, `iag`, `pwconv`, `cat` aligned to src_jasa
    ...
    def __init__(
        ...
        self.skip_pathway_up = nn.ModuleList()
        for skip_channels, gate_channels in zip(self._skip_channels, self._decoder_gate_channels):
            if self.cfg.skip_mode == "ag":
                self.skip_pathway_up.append(
                    AttentionGate(
                        decoder_channels=gate_channels,
                        encoder_channels=skip_channels,
                        cfg=attention_gate_cfg,
                    )
                )
            elif self.cfg.skip_mode == "iag":
                if skip_channels != gate_channels:
                    raise ValueError(
                        f"iag requires equal decoder/encoder channels, got decoder={gate_channels}, encoder={skip_channels}."
                    )
                self.skip_pathway_up.append(ImprovedAttentionGate(skip_channels))
            elif self.cfg.skip_mode == "pwconv":
                self.skip_pathway_up.append(nn.Conv2d(skip_channels, skip_channels, kernel_size=1))
            elif self.cfg.skip_mode == "cat":
                self.skip_pathway_up.append(IdentitySkip())
            else:
                raise ValueError(f"Unsupported skip_mode={self.cfg.skip_mode!r}. Expected ag/iag/pwconv/cat.")
    ...
    def _validate_config(self) -> None:
        ...
        if self.cfg.skip_mode not in {"ag", "iag", "pwconv", "cat"}:
            raise ValueError("DenGCAN skip_mode must be one of {'ag', 'iag', 'pwconv', 'cat'}.")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for idx, (
            ...
            if self.cfg.skip_mode in {"ag", "iag"}:
                gated_skip = skip_path(cur, skip)
            else:
                gated_skip = skip_path(skip)
